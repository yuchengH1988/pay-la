...
export function ExpenseHistory({
  ...
  const { expenses, loading, error } = useExpenses(group.id);
  ...
  return (
    <Frame as="section" className="p-5">
      <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="type-h2">Expense history</h2>
          <p className="type-small mt-2 text-muted">
            Balance calculation starts in the next phase.
          </p>
        </div>
        <Badge tone="muted">{expenses.length} records</Badge>
      </div>
